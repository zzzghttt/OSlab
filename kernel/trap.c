...
usertrap(void)
{
  ...
  if(r_scause() == 8){
    ...
    int offset = PGROUNDDOWN(addr) - vma->addr + vma->offset;
    ilock(fp->ip);
    if(readi(fp->ip, 0, (uint64)mem, offset, PGSIZE) == 0){
      iunlock(fp->ip);
      exit(-1);
    }
...
int 
unmap(struct proc *p, uint64 addr, int length)
{
  struct vm_area *vma = p->vm_area_list;
  while(vma){
    if(vma->addr <= addr && addr < vma->addr+vma->length)
